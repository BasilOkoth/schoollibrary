...
def mpesa_subscription_callback(request):
    """
    Temporary Safaricom callback for ShuleHub school subscription payments.

    This version does not use SchoolSubscriptionPayment because that model
    has not yet been created. It safely accepts callbacks without crashing
    the application.
    """
    try:
        payload = json.loads(request.body.decode("utf-8"))
    ...
    logger.info(
        "Subscription M-Pesa callback received. CheckoutRequestID=%s ResultCode=%s ResultDesc=%s",
        checkout_request_id,
        result_code,
        result_desc,
    )
