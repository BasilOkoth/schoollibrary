...
from .models import Class, Exam, Student, StudentResult, Subject, StudentSubject
...
def print_job_detail(request, job_id=None, tenant_schema=None, *args, **kwargs):
    ...
    with schema_context(schema_name):
        ...
        if (
            user_role in ["secretary", "admin", "principal"]
            or job.teacher == request.user
        ):
...
@login_required
def bulk_download_generated_certificates(request, tenant_schema=None):
    """
    Download already-generated certificate PDFs for an exam/class as a ZIP.

    This does NOT generate new certificates.
    It only collects PDFs already saved in GeneratedCertificate.
    """
    import io
    import os
    import zipfile
    ...
    from .models import Exam, Class, GeneratedCertificate
    ...
    if class_id:
        ...
        certificates = certificates.filter(student_class=selected_class)
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for cert in certificates:
            ...
            exam_name = slugify(exam.name)
            file_name = f"{student_name}-{exam_name}-certificate.pdf"

            try:
                cert.certificate_file.open("rb")
                file_bytes = cert.certificate_file.read()
    ...
    if added_count == 0:
        messages.error(
            request,
            "Certificate records were found, but the PDF files could not be read.",
        )
        return redirect(f"{tenant_base_url}/certificates/bulk/")
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="{class_slug}-{exam_slug}-generated-certificates-{date_slug}.zip"'
    )
...
def tv_display(request, tenant_schema=None, *args, **kwargs):
    """
    Public TV display page.

    This view must be accessible without login.
    It uses the tenant schema from the URL:
    /tenant/<tenant_schema>/app/tv/
    """
    ...
    # ------------------------------------------------------------
    # 1. Resolve tenant schema from URL/request
    # ------------------------------------------------------------
    schema_name = (
        tenant_schema
        or getattr(request, "tenant_schema", None)
    ...
    # ------------------------------------------------------------
    # 2. Get school name from public schema, for display only
    # ------------------------------------------------------------
    school_name = f"{schema_name.title()} School"
    ...
    with schema_context(schema_name):
        ...
        return render(
            request,
            "digitallibrary/tv/display.html",
            context,
        )
@login_required
...
def home(request, tenant_schema=None):
    ...
    if current_schema == "public" and not tenant_schema:
        ...
        # Public announcements may not exist in public schema.
...
def set_grading_preference(request, tenant_schema=None, exam_id=None):
    ...
    Form 3 and Form 4 are old curriculum classes and should use
    the school grading system, not CBE.
    """
    ...
    with schema_context(schema_name):
        ...
        # ------------------------------------------------------------
        # Preserve subject and class selection
        # ------------------------------------------------------------
        subject_id = (
            request.GET.get("subject")
            or request.POST.get("subject_id")
        ...
        if request.method == "POST":
            ...
            # ----------------------------------------------------
            # OLD CURRICULUM: force school/traditional grading
            # ----------------------------------------------------
            if old_curriculum:
            ...
            # ----------------------------------------------------
            # 2. KCSE / traditional / default school grading
            # ----------------------------------------------------
        ...
        # ------------------------------------------------------------
        # Tenant-safe redirect back to results entry
        # Important: preserve class_id and subject_id.
        # ------------------------------------------------------------
        redirect_url = (
...
def download_stream_completion_status(
    ...
    **kwargs,
    ...
    with schema_context(schema_name):
        ...
        return response
# ============================================================
# ADD THIS TO digitallibrary/views.py
# ============================================================
# This creates the two routes used by the Class Teacher Dashboard:
# /teacher/class/download-combined/<exam_id>/
# /teacher/class/download-stream-status/<exam_id>/
#
# No migration needed.
# ============================================================
...
def _safe_student_name(student):
    first = getattr(student, "first_name", "") or ""
    last = getattr(student, "last_name", "") or ""
    full = f"{first} {last}".strip()
    return full or str(student)


def _safe_stream_name(student):
    for field in ["stream", "class_stream", "student_stream"]:
...
def _get_class_for_download(request, Class, class_id=None):
    if class_id:
        return get_object_or_404(Class, id=class_id)
    ...
    try:
        ClassTeacherAssignment = apps.get_model("digitallibrary", "ClassTeacherAssignment")
    except LookupError:
        ClassTeacherAssignment = None

    if ClassTeacherAssignment:
        assignment = (
            ClassTeacherAssignment.objects.filter(class_teacher=request.user)
            .select_related("class_obj")
            .first()
        )
        if assignment:
...
def download_combined_class_results(request, exam_id, class_id=None, tenant_schema=None, *args, **kwargs):
    ...
    with schema_context(schema_name):
        exam = get_object_or_404(Exam, id=exam_id)
        target_class = _get_class_for_download(request, Class, class_id=class_id)

        response = _csv_response(f"combined_results_exam_{exam_id}.csv")
        writer = csv.writer(response)

        if not target_class:
            writer.writerow(["No assigned class found for this user."])
            writer.writerow(["Assign the teacher to a class or stream first."])
            return response

        ResultModel = _find_result_model()

        writer.writerow(["Combined Class Results"])
        writer.writerow(["Class", str(target_class)])
        writer.writerow(["Exam", getattr(exam, "name", str(exam))])
        writer.writerow(["Academic Year", getattr(exam, "academic_year", "")])
        writer.writerow(["Term", getattr(exam, "term", "")])
        writer.writerow([])
        writer.writerow([
        ...
        students = (
            Student.objects.filter(current_class=target_class, is_active=True)
        ...
        for student in students:
            ...
            if ResultModel:
                fields = {f.name for f in ResultModel._meta.get_fields()}
                results = ResultModel.objects.all()

                if "exam" in fields:
                    results = results.filter(exam=exam)
                elif "exam_id" in fields:
                    results = results.filter(exam_id=exam.id)

                if "student" in fields:
                    results = results.filter(student=student)
                elif "student_id" in fields:
                    results = results.filter(student_id=student.id)

                for result in results:
                    total_score += _score_from_result(result)
                    subject_count += 1

            average = total_score / subject_count if subject_count else 0.0

            rows.append({
                "student": student,
                "stream": _safe_stream_name(student),
                "total": total_score,
                "average": average,
...
def download_stream_completion_status(request, exam_id, class_id=None, tenant_schema=None, *args, **kwargs):
    ...
    with schema_context(schema_name):
        ...
        response = _csv_response(f"stream_status_exam_{exam_id}.csv")
        writer = csv.writer(response)
        ...
        students = Student.objects.filter(current_class=target_class, is_active=True)

        streams = defaultdict(list)
        for student in students:
            streams[_safe_stream_name(student) or "No Stream"].append(student)

        writer.writerow(["Stream Completion Status"])
        writer.writerow(["Class", str(target_class)])
        writer.writerow(["Exam", getattr(exam, "name", str(exam))])
        writer.writerow([])
        writer.writerow(["Stream", "Students", "Students With Results", "Completion %", "Status"])
        ...
        for stream_name, stream_students in sorted(streams.items()):
            ...
            if ResultModel:
                fields = {f.name for f in ResultModel._meta.get_fields()}
                ids = [s.id for s in stream_students]
                ...
                if "student" in fields:
                    results = results.filter(student_id__in=ids)
                    completed_ids = set(results.values_list("student_id", flat=True).distinct())
                elif "student_id" in fields:
                    results = results.filter(student_id__in=ids)
                    completed_ids = set(results.values_list("student_id", flat=True).distinct())
            ...
            writer.writerow([
                ...
                completed,
                completion,
                "Ready" if total and completion == 100 else "In progress",
            ])
