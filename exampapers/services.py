...
def validate_component_configuration(
    ...
    for component in components:
        if not papers_by_component.get(
            component.id
        ):
            raise PaperMarkError(
                f"{component.name} does not "
                "contain any papers."
            )
    ...
    *,
    ...
    for component in components:
        ...
        for paper in component_papers:
            if (
                paper.id
                not in scores_by_paper_id
            ):
                raise PaperMarkError(
                    f"Missing "
                    f"{paper.paper_name} mark."
                )
    ...
    if (
        final_percentage < 0
        or final_percentage > ONE_HUNDRED
    ):
        raise PaperMarkError(
            "The calculated final result "
...
def calculate_existing_subject_score(
    ...
    existing_marks = {
        mark.paper_id: mark
        for mark
        in StudentPaperMark.objects.filter(
            student=student,
            paper__in=papers,
        )
    }
...
def save_student_paper_marks(
    ...
    if unknown_paper_ids:
        raise PaperMarkError(
            "One or more submitted papers "
            "do not belong to this exam "
            "and subject."
        )
